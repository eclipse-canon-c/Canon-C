 ...
 *
 * Performance:
 * - Time complexity: O(n) where n = file size or content length
 * - Space complexity: O(n) where n = file size or content length
 *   (excluding fixed-size scratch space / control structures)
 * - Arena operations: zero additional heap allocations
 * - Heap operations: O(1) heap allocations per call (usually 1)
 * - I/O pattern: single sequential read or write pass
 *
 * Core ideas:
 * ────────────────────────────────────────────────────────────────────────────
 ...
 * - Space complexity: O(n) from arena (plus O(1) stack / control structures)
 * - I/O operations: 2 seeks + 1 full read
 ...
 * Performance:
 * - Time complexity: O(n) where n = file size
 * - Space complexity: O(n) on heap + O(1) temporary stack scratch space
 * - Allocations: 1 stack arena initialization + 1 heap allocation
 * - Contains one full copy (arena → heap)
 *
 * Limitations:
 * - Current implementation uses 4 KiB scratch arena
 * - Files > ~4095 bytes will fail unless scratch size is increased
 * - For larger files prefer file_read_all_arena() with bigger arena
 *
 * Memory ownership:
 ...
 *
 * Performance:
 * - Time complexity: O(n) where n = strlen(content)
 * - Space complexity: O(1) — no allocations
 * - I/O operations: 1 write + close/flush
 *
...
void test_parser_with_fixture(void) {
    ...
    option_charp fixture = file_read_all_arena("tests/fixtures/valid.xml", &arena);
