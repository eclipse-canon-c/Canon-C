...
static inline uint32_t random_u32(Random* r) {
    ...
    // Safe branchless rotate-right (avoids UB when rot=0 or shift amount negative)
...
static inline uint32_t random_range(Random* r, uint32_t bound) {
    ...
    // Portable calculation of threshold (size of the biased zone)
    // Equivalent to -bound % bound but always safe on unsigned arithmetic
...
/* ────────────────────────────────────────────────────────────────────────────
   ...
   ────────────────────────────────────────────────────────────────────────────
    ...
    void shuffle_array(int* arr, size_t n, Random* rng) {
        if (n <= 1) return; // safe: no-op on empty or single-element array
        for (size_t i = n - 1; i > 0; --i) {
            size_t j = random_range(rng, i + 1);
            int temp = arr[i];
            arr[i] = arr[j];
    ...
    int random_int_range(Random* r, int min, int max) {
        if (min >= max) return min; // safe: return min if invalid range
        uint32_t range = (uint32_t)(max - min + 1); // safe for int32_t range
        return min + (int)random_range(r, range);
    }
