...
static void test_init_sets_state(void)
{
    ...
    EXPECT(vec_int_len(&v)       == 0);
    EXPECT(vec_int_capacity(&v)  == 8);
    EXPECT(vec_int_remaining(&v) == 8);
    EXPECT(vec_int_is_empty(&v));
    EXPECT(!vec_int_is_full(&v));
}
...
static void test_empty_is_null_state(void)
{
    ...
    EXPECT(vec_int_len(&v)       == 0);
    EXPECT(vec_int_capacity(&v)  == 0);
    EXPECT(vec_int_remaining(&v) == 0);
    EXPECT(vec_int_is_empty(&v));
    EXPECT(vec_int_is_full(&v)); /* NULL-guard: is_full returns true */
}
...
static void test_alloc_and_free(void)
{
    ...
    EXPECT(v.items != NULL);
    EXPECT(vec_int_capacity(&v) == 4);
    EXPECT(vec_int_len(&v)      == 0);

    vec_int_push(&v, 1);
    vec_int_push(&v, 2);
    EXPECT(vec_int_len(&v) == 2);
...
{
    EXPECT(vec_int_len(NULL)       == 0);
    EXPECT(vec_int_capacity(NULL)  == 0);
    EXPECT(vec_int_remaining(NULL) == 0);
    EXPECT(vec_int_is_empty(NULL));
    EXPECT(vec_int_is_full(NULL));
}
...
{
    int buf[2];
    vec_int v = vec_int_init(buf, 2);
    vec_int_push(&v, 1);
    vec_int_push(&v, 2);

    EXPECT(vec_int_is_full(&v));

    result__Bool_Error r = vec_int_push(&v, 3);
    EXPECT(!result__Bool_Error_is_ok(r));
    EXPECT(result__Bool_Error_unwrap_err(r) == ERR_CAPACITY_EXCEEDED);
    EXPECT(vec_int_len(&v) == 2); /* unchanged */
}

static void test_push_null_vec_returns_err(void)
{
    result__Bool_Error r = vec_int_push(NULL, 1);
    EXPECT(!result__Bool_Error_is_ok(r));
...
static void test_try_push_returns_bool(void)
{
    ...
    EXPECT(vec_int_try_push(&v, 1));
    EXPECT(vec_int_try_push(&v, 2));
    EXPECT(!vec_int_try_push(&v, 3)); /* full */
    EXPECT(vec_int_len(&v) == 2);
}
...
static void test_pop_removes_last(void)
{
    ...
    int out = 0;
    result__Bool_Error r = vec_int_pop(&v, &out);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(out == 3);
    EXPECT(vec_int_len(&v) == 2);
...
{
    int out = 0;
    result__Bool_Error r = vec_int_pop(NULL, &out);
    EXPECT(!result__Bool_Error_is_ok(r));
    EXPECT(result__Bool_Error_unwrap_err(r) == ERR_INVALID_ARG);
}
...
static void test_pop_null_out_returns_err(void)
{
    ...
    result__Bool_Error r = vec_int_pop(&v, NULL);
    EXPECT(!result__Bool_Error_is_ok(r));
    EXPECT(result__Bool_Error_unwrap_err(r) == ERR_INVALID_ARG);
}
...
static void test_pop_option_some(void)
{
    ...
    option_int o = vec_int_pop_option(&v);
...
static void test_get_reads_element(void)
{
    ...
    int out = 0;
    EXPECT(vec_int_get(&v, 0, &out));
    EXPECT(out == 10);

    EXPECT(vec_int_get(&v, 1, &out));
    EXPECT(out == 20);

    EXPECT(!vec_int_get(&v, 2, &out)); /* out of range */
}
...
static void test_get_option_some_and_none(void)
{
    ...
    option_int o = vec_int_get_option(&v, 0);
    EXPECT(option_int_is_some(o));
    EXPECT(option_int_unwrap(o) == 7);

    option_int n = vec_int_get_option(&v, 1);
...
static void test_set_overwrites_element(void)
{
    ...
    EXPECT(vec_int_set(&v, 0, 99));
    int out = 0;
    vec_int_get(&v, 0, &out);
    EXPECT(out == 99);

    EXPECT(!vec_int_set(&v, 2, 0)); /* out of range — len==2, so index 2 is invalid */
}
...
static void test_first_last_data(void)
{
    ...
    EXPECT(vec_int_first(&v) == NULL); /* empty */
    EXPECT(vec_int_last(&v)  == NULL);

    vec_int_push(&v, 10);
    vec_int_push(&v, 20);
    vec_int_push(&v, 30);

    EXPECT_NOT_NULL(vec_int_first(&v));
    EXPECT(*vec_int_first(&v) == 10);

    EXPECT_NOT_NULL(vec_int_last(&v));
    EXPECT(*vec_int_last(&v) == 30);

    EXPECT(vec_int_data(&v) == buf);
}
...
{
    int buf[5];
    vec_int v = vec_int_init(buf, 5);
    vec_int_push(&v, 1);
    vec_int_push(&v, 2);
    vec_int_push(&v, 3);

    /* Insert at index 1: [1, 99, 2, 3] */
    result__Bool_Error r = vec_int_insert(&v, 1, 99);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(vec_int_len(&v) == 4);

    int out = 0;
    vec_int_get(&v, 0, &out); EXPECT(out == 1);
    vec_int_get(&v, 1, &out); EXPECT(out == 99);
    vec_int_get(&v, 2, &out); EXPECT(out == 2);
    vec_int_get(&v, 3, &out); EXPECT(out == 3);
}
...
static void test_insert_at_end_appends(void)
{
    ...
    result__Bool_Error r = vec_int_insert(&v, 2, 3);
    EXPECT(result__Bool_Error_is_ok(r));
...
static void test_insert_out_of_range_returns_err(void)
{
    ...
    result__Bool_Error r = vec_int_insert(&v, 5, 99);
    EXPECT(!result__Bool_Error_is_ok(r));
...
static void test_insert_full_returns_err(void)
{
    ...
    result__Bool_Error r = vec_int_insert(&v, 0, 99);
    EXPECT(!result__Bool_Error_is_ok(r));
    EXPECT(result__Bool_Error_unwrap_err(r) == ERR_CAPACITY_EXCEEDED);
}
...
static void test_remove_shifts_left(void)
{
    ...
    int out = 0;
    result__Bool_Error r = vec_int_remove(&v, 1, &out);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(out == 2);
    EXPECT(vec_int_len(&v) == 2);

    vec_int_get(&v, 0, &out); EXPECT(out == 1);
    vec_int_get(&v, 1, &out); EXPECT(out == 3);
}
...
static void test_remove_last_element(void)
{
    ...
    int out = 0;
    vec_int_remove(&v, 1, &out);
    EXPECT(out == 2);
    EXPECT(vec_int_len(&v) == 1);
}
...
static void test_remove_out_of_range_returns_err(void)
{
    ...
    int out = 0;
    result__Bool_Error r = vec_int_remove(&v, 5, &out);
    EXPECT(!result__Bool_Error_is_ok(r));
    EXPECT(result__Bool_Error_unwrap_err(r) == ERR_OUT_OF_RANGE);
}
...
static void test_remove_empty_returns_err(void)
{
    ...
    int out = 0;
    result__Bool_Error r = vec_int_remove(&v, 0, &out);
    EXPECT(!result__Bool_Error_is_ok(r));
    EXPECT(result__Bool_Error_unwrap_err(r) == ERR_INVALID_STATE);
}
...
static void test_remove_option_some(void)
{
    ...
    option_int o = vec_int_remove_option(&v, 0);
    EXPECT(option_int_is_some(o));
...
static void test_remove_option_none_out_of_range(void)
{
    ...
    option_int o = vec_int_remove_option(&v, 5);
    EXPECT(option_int_is_none(o));
}
...
static void test_append_array_copies_elements(void)
{
    ...
    int src[] = {1, 2, 3, 4};
    result__Bool_Error r = vec_int_append_array(&v, src, 4);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(vec_int_len(&v) == 4);

    int out = 0;
    vec_int_get(&v, 0, &out); EXPECT(out == 1);
    vec_int_get(&v, 3, &out); EXPECT(out == 4);
}
...
static void test_append_array_capacity_exceeded(void)
{
    ...
    int src[] = {1, 2, 3};
    result__Bool_Error r = vec_int_append_array(&v, src, 3);
    EXPECT(!result__Bool_Error_is_ok(r));
    EXPECT(result__Bool_Error_unwrap_err(r) == ERR_CAPACITY_EXCEEDED);
}
...
static void test_extend_is_alias_for_append_array(void)
{
    ...
    int src[] = {5, 6};
    result__Bool_Error r = vec_int_extend(&v, src, 2);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(vec_int_len(&v) == 2);
}
...
{
    int buf[6];
    vec_int v = vec_int_init(buf, 6);
    vec_int_push(&v, 0); /* pre-existing element */

    vec_int_fill(&v, 7, 3);

    EXPECT(vec_int_len(&v) == 4);
    int out = 0;
    vec_int_get(&v, 1, &out); EXPECT(out == 7);
    vec_int_get(&v, 2, &out); EXPECT(out == 7);
    vec_int_get(&v, 3, &out); EXPECT(out == 7);
}

static void test_fill_truncates_at_capacity(void)
{
    int buf[3];
    vec_int v = vec_int_init(buf, 3);
...
static void test_clear_allows_reuse(void)
{
    ...
    result__Bool_Error r = vec_int_push(&v, 99);
    EXPECT(result__Bool_Error_is_ok(r));
}

static void test_clear_null_safe(void)
{
    vec_int_clear(NULL); /* must not crash */
}
...
static void test_swap_exchanges_vecs(void)
{
    ...
    EXPECT(vec_int_len(&a) == 1);
    EXPECT(vec_int_len(&b) == 2);

    int out = 0;
    vec_int_get(&a, 0, &out); EXPECT(out == 9);
    vec_int_get(&b, 0, &out); EXPECT(out == 1);
    vec_int_get(&b, 1, &out); EXPECT(out == 2);
}
...
static void test_len_remaining_invariant(void)
{
    ...
    for (int i = 0; i < 5; i++) {
        EXPECT(vec_int_len(&v) + vec_int_remaining(&v)
               == vec_int_capacity(&v));
        vec_int_push(&v, i);
    }
    EXPECT(vec_int_len(&v) + vec_int_remaining(&v)
...
static void test_iter_empty_vec(void)
{
    ...
    int val = 0;
    EXPECT(!vec_int_iter_next(&it, &val));
}
...
static void test_slice_init_sub_range(void)
{
    int buf[6];
    vec_int v = vec_int_init(buf, 6);
    for (int i = 0; i < 6; i++) vec_int_push(&v, i * 10);

    /* Slice of elements [1, 3) → values 10, 20 */
    vec_int_slice s = vec_int_slice_init(&v, 1, 3);
    EXPECT(s.len == 2);
    EXPECT_NOT_NULL(s.items);
    EXPECT(*vec_int_slice_get(&s, 0) == 10);
    EXPECT(*vec_int_slice_get(&s, 1) == 20);
}
...
static void test_slice_invalid_range_returns_empty(void)
{
    ...
    vec_int_slice s = vec_int_slice_init(&v, 2, 1); /* start > end */
    EXPECT(s.len == 0);
}
...
static void test_slice_full_vec(void)
{
    ...
    vec_int_slice s = vec_int_slice_init(&v, 0, 3);
    EXPECT(s.len == 3);
    EXPECT(*vec_int_slice_get(&s, 2) == 3);
}
...
{
    Point buf[4];
    vec_Point v = vec_Point_init(buf, 4);
    ...
    /* empty() */
    vec_Point empty_v = vec_Point_empty();
    EXPECT(vec_Point_len(&empty_v)      == 0);
    EXPECT(vec_Point_capacity(&empty_v) == 0);

    /* push / len / capacity / remaining / is_empty / is_full */
    vec_Point_push(&v, a);
    vec_Point_push(&v, b);
    vec_Point_push(&v, c);
    EXPECT(vec_Point_len(&v)       == 3);
    EXPECT(vec_Point_remaining(&v) == 1);
    EXPECT(!vec_Point_is_empty(&v));
    EXPECT(!vec_Point_is_full(&v));

    /* get */
    Point out = zero;
    EXPECT(vec_Point_get(&v, 0, &out));
    EXPECT(point_eq(out, a));

    /* get_option */
    option_Point go = vec_Point_get_option(&v, 1);
    EXPECT(option_Point_is_some(go));
    EXPECT(point_eq(option_Point_unwrap(go), b));
    EXPECT(option_Point_is_none(vec_Point_get_option(&v, 99)));

    /* get_unchecked */
    EXPECT(point_eq(vec_Point_get_unchecked(&v, 2), c));

    /* at / set */
    Point* ptr = vec_Point_at(&v, 0);
    EXPECT_NOT_NULL(ptr);
    EXPECT(point_eq(*ptr, a));
    EXPECT(vec_Point_set(&v, 0, b));
    vec_Point_get(&v, 0, &out);
    EXPECT(point_eq(out, b));
    vec_Point_set(&v, 0, a); /* restore */

    /* first / last / data */
    EXPECT_NOT_NULL(vec_Point_first(&v));
    EXPECT(point_eq(*vec_Point_first(&v), a));
    EXPECT_NOT_NULL(vec_Point_last(&v));
    EXPECT(point_eq(*vec_Point_last(&v), c));
    EXPECT(vec_Point_data(&v) == buf);

    /* try_push */
    EXPECT(vec_Point_try_push(&v, zero)); /* fills slot 4 */
    EXPECT(vec_Point_is_full(&v));
    EXPECT(!vec_Point_try_push(&v, zero)); /* full */

    /* pop_option */
    option_Point po = vec_Point_pop_option(&v);
    EXPECT(option_Point_is_some(po));
    EXPECT(point_eq(option_Point_unwrap(po), zero));

    /* pop (Result) */
    result__Bool_Error r = vec_Point_pop(&v, &out);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(point_eq(out, c));
    EXPECT(vec_Point_len(&v) == 2); /* a, b remain */

    /* insert */
    r = vec_Point_insert(&v, 1, c);
    EXPECT(result__Bool_Error_is_ok(r));
    vec_Point_get(&v, 1, &out);
    EXPECT(point_eq(out, c));

    /* remove */
    r = vec_Point_remove(&v, 1, &out);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(point_eq(out, c));

    /* remove_option */
    option_Point ro = vec_Point_remove_option(&v, 0);
    EXPECT(option_Point_is_some(ro));
    EXPECT(point_eq(option_Point_unwrap(ro), a));

    /* append_array */
    Point src[2] = {{10, 20}, {30, 40}};
    r = vec_Point_append_array(&v, src, 2);
    EXPECT(result__Bool_Error_is_ok(r));

    /* extend */
    vec_Point_clear(&v);
    r = vec_Point_extend(&v, src, 2);
    EXPECT(result__Bool_Error_is_ok(r));
    EXPECT(vec_Point_len(&v) == 2);

    /* fill */
    vec_Point_clear(&v);
    vec_Point_fill(&v, zero, 3);
    EXPECT(vec_Point_len(&v) == 3);

    /* swap */
    Point buf2[4];
    vec_Point v2 = vec_Point_init(buf2, 4);
    vec_Point_push(&v2, a);
    vec_Point_swap(&v, &v2);
    EXPECT(vec_Point_len(&v) == 1);
    vec_Point_get(&v, 0, &out);
    EXPECT(point_eq(out, a));
    ...
    vec_Point_push(&v, a);
    vec_Point_push(&v, b);
    vec_Point_iter it = vec_Point_iter_init(&v);
    Point itval = zero;
    int itcount = 0;
    while (vec_Point_iter_next(&it, &itval)) { itcount++; }
    EXPECT(itcount == 2);

    /* slice */
    vec_Point_slice s = vec_Point_slice_init(&v, 0, 2);
    EXPECT(s.len == 2);
    EXPECT_NOT_NULL(vec_Point_slice_get(&s, 0));

    /* push_unchecked */
    vec_Point_clear(&v);
    vec_Point_push_unchecked(&v, a);
    EXPECT(vec_Point_len(&v) == 1);

    /* free (heap-allocated) */
    vec_Point heap_v = vec_Point_alloc(2);
    EXPECT(heap_v.items != NULL);
    vec_Point_push(&heap_v, a);
    vec_Point_free(&heap_v);
    EXPECT(heap_v.items == NULL);
}
...
static void vec_fuzz_suppress_unused(void)
{
    ...
    (void)option_Point_eq;
    (void)vec_Point_init;
    (void)vec_Point_empty;
    (void)vec_Point_alloc;
    (void)vec_Point_arena_alloc;
    (void)vec_Point_free;
    (void)vec_Point_len;
    (void)vec_Point_capacity;
    (void)vec_Point_remaining;
    (void)vec_Point_is_empty;
    (void)vec_Point_is_full;
    (void)vec_Point_get;
    (void)vec_Point_get_option;
    (void)vec_Point_get_unchecked;
    (void)vec_Point_at;
    (void)vec_Point_set;
    (void)vec_Point_first;
    (void)vec_Point_last;
    (void)vec_Point_data;
    (void)vec_Point_push;
    (void)vec_Point_try_push;
    (void)vec_Point_push_unchecked;
    (void)vec_Point_pop;
    (void)vec_Point_pop_option;
    (void)vec_Point_clear;
    (void)vec_Point_insert;
    (void)vec_Point_remove;
    (void)vec_Point_remove_option;
    (void)vec_Point_append_array;
    (void)vec_Point_extend;
    (void)vec_Point_fill;
    (void)vec_Point_swap;
    (void)vec_Point_iter_init;
...
int LLVMFuzzerTestOneInput(const u8 *data, usize size)
{
    ...
    for (usize i = 1; i < size; i++) {
        ...
        #define CHECK_INVARIANTS()                                            \
            do {                                                              \
                usize len = vec_int_len(&v);                            \
                usize rem = vec_int_remaining(&v);                      \
                if (len + rem != cap)                   __builtin_trap();     \
                if (vec_int_is_empty(&v) != (len == 0))                \
                                                        __builtin_trap();     \
                if (vec_int_is_full(&v)  != (len == cap))              \
                                                        __builtin_trap();     \
            } while (0)

        switch (op) {
            case 0: { /* push */
                usize before = vec_int_len(&v);
                result__Bool_Error r = vec_int_push(&v, val);
                if (before >= cap) {
            ...
            case 1: { /* pop */
                ...
                if (result__Bool_Error_is_ok(r)) {
                    if (vec_int_is_empty(&v) &&
                        vec_int_len(&v) != 0)     __builtin_trap();
                    ...
                    if (vec_int_len(&v) != 0)     __builtin_trap();
                }
                break;
            }
            case 2: { /* insert at position */
                ...
                result__Bool_Error r = vec_int_insert(&v, idx, val);
                if (len >= cap) {
                    if (result__Bool_Error_is_ok(r))    __builtin_trap();
                } else {
                    if (!result__Bool_Error_is_ok(r))   __builtin_trap();
            ...
            case 3: { /* remove */
                ...
                int out = 0;
                result__Bool_Error r = vec_int_remove(&v, idx, &out);
                if (!result__Bool_Error_is_ok(r))       __builtin_trap();
            ...
            case 4: { /* get */
                ...
                usize idx = (usize)val % len;
                int out = 0;
                if (!vec_int_get(&v, idx, &out))  __builtin_trap();
                /* get must not mutate len */
                if (vec_int_len(&v) != len)       __builtin_trap();
                break;
            }
            case 5: { /* set */
                usize len = vec_int_len(&v);
                if (len == 0) break;
                usize idx = (usize)val % len;
                if (!vec_int_set(&v, idx, val + 1)) __builtin_trap();
                if (vec_int_len(&v) != len)         __builtin_trap();
            ...
            case 6: { /* clear */
                vec_int_clear(&v);
                if (!vec_int_is_empty(&v))        __builtin_trap();
                if (vec_int_len(&v) != 0)         __builtin_trap();
                ref_top = 0;
                break;
            }
