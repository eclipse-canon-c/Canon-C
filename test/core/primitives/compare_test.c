...
/* Helpers: sign of an int */
static int sign(int v) { return (v > 0) - (v < 0); }

/* =========================================================================
 * Helper macros
 *
 * CONTRACT3(fn, lo, hi, mid) — verifies the three fundamental relations
 * that every total-order comparator must satisfy for three distinct values
 * where lo < mid < hi in the ascending ordering:
 *
 *   fn(&lo,  &lo,  NULL) == 0   (equal)
 *   fn(&hi,  &lo,  NULL) > 0   (greater)
 *   fn(&lo,  &hi,  NULL) < 0   (less)
 *   fn(&mid, &lo,  NULL) > 0
 *   fn(&lo,  &mid, NULL) < 0
 *   fn(&mid, &hi,  NULL) < 0
 *   fn(&hi,  &mid, NULL) > 0
 *   antisymmetry: sign(fn(a,b)) == -sign(fn(b,a)) for all pairs
 *
 * DESC_CONTRACT(asc_fn, desc_fn, a, b) — verifies that the descending
 * variant reverses the ascending result for every non-equal pair.
 * ====================================================================== */
...
#define CONTRACT3(fn, lo_val, hi_val, mid_val)                              \
    do {                                                                    \
        ...
        /* antisymmetry */                                                  \
        EXPECT(sign(fn(&_lo,  &_hi,  NULL)) ==                              \
              -sign(fn(&_hi,  &_lo,  NULL)));                               \
        EXPECT(sign(fn(&_lo,  &_mid, NULL)) ==                              \
              -sign(fn(&_mid, &_lo,  NULL)));                               \
        EXPECT(sign(fn(&_mid, &_hi,  NULL)) ==                              \
              -sign(fn(&_hi,  &_mid, NULL)));                               \
    } while (0)
...
#define DESC_REVERSES(asc_fn, desc_fn, val_a, val_b)                        \
    do {                                                                    \
        ...
        /* desc reverses asc for non-equal values */                        \
        if (_asc != 0) {                                                    \
            EXPECT(sign(_asc) == -sign(_desc));                             \
        } else {                                                            \
            EXPECT(_desc == 0);                                             \
        }                                                                   \
    } while (0)
...
TEST(cmp_u8) {
    ...
    CONTRACT3(algo_cmp_u8, (u8)1,   (u8)2,   (u8)0);  /* note: lo=1, hi=2, mid=0 — reorder */
    /* Boundary */
...
TEST(cmp_usize) {
    ...
    /* isize_min computation without overflow */
    {
...
TEST(cmp_f32) {
    ...
    /* NaN sorted last */
    EXPECT(algo_cmp_f32(&nan1, &pos,  NULL) > 0);   /* NaN > any non-NaN */
    EXPECT(algo_cmp_f32(&pos,  &nan1, NULL) < 0);   /* non-NaN < NaN */
    EXPECT(algo_cmp_f32(&nan1, &inf,  NULL) > 0);   /* NaN > INFINITY */
    EXPECT(algo_cmp_f32(&inf,  &nan1, NULL) < 0);
    EXPECT(algo_cmp_f32(&nan1, &nan2, NULL) == 0);  /* NaN == NaN */
    ...
    /* desc: NaN sorted first (NaN was last in asc, so first in desc) */
...
TEST(cmp_f64) {
    ...
    DESC_REVERSES(algo_cmp_f64, algo_cmp_f64_desc, INFINITY, 1.0);
...
TEST(total_order_properties) {
    /* Antisymmetry: sign(cmp(a,b)) == -sign(cmp(b,a)) for all integer types */
    {
        u32 lo = 1, hi = 2;
        EXPECT(sign(algo_cmp_u32(&lo, &hi, NULL)) ==
              -sign(algo_cmp_u32(&hi, &lo, NULL)));
        EXPECT(sign(algo_cmp_u32(&lo, &lo, NULL)) == 0);
    }
    {
        i32 lo = -5, hi = 5;
        EXPECT(sign(algo_cmp_i32(&lo, &hi, NULL)) ==
              -sign(algo_cmp_i32(&hi, &lo, NULL)));
    }
...
TEST(desc_reverses_asc) {
    ...
    DESC_REVERSES(algo_cmp_i32,   algo_cmp_i32_desc,   (i32)(-2147483647 - 1), (i32)2147483647);
    DESC_REVERSES(algo_cmp_i64,   algo_cmp_i64_desc,   (i64)(-9223372036854775807LL - 1),
                                                        (i64)9223372036854775807LL);
    ...
    /* Equal values produce 0 in both asc and desc */
...
int LLVMFuzzerTestOneInput(const unsigned char *data, size_t size) {
    ...
    /* Test all integer comparators with the raw bytes reinterpreted */
    {
        ...
        /* Antisymmetry invariant for every type */
#define CHECK_ANTISYM(fn, pa, pb) \
        if (sign(fn(pa, pb, NULL)) != -sign(fn(pb, pa, NULL))) __builtin_trap()
...
#undef CHECK_ANTISYM
        ...
        /* desc reverses asc for every integer type */
#define CHECK_DESC(asc, desc, pa, pb)                                  \
        {                                                               \
            int _a = asc(pa, pb, NULL);                                \
            int _d = desc(pa, pb, NULL);                               \
            if (_a != 0 && sign(_a) != -sign(_d)) __builtin_trap();    \
...
#undef CHECK_DESC
    ...
    /* Float comparators: reinterpret bytes as f32/f64 bit patterns */
    {
        f32 af32, bf32;
        f64 af64, bf64;
