 ...
 * Dependency rule:
 * ────────────────────────────────────────────────────────────────────────────
 * data/bitset.h is data/. It depends only on core/.
 ...
 * ```
 *
 * @sa core/primitives/bits.h — popcount64, ctz64 used internally
 ...
 */
static inline void bitset_clear_padding(Bitset* bs) {
 ...
 *
 * @pre bs != NULL, words != NULL, capacity > 0
 * @post All bits cleared, bs ready to use
 *
 * Performance: O(n/64) — clears backing words
 */
static inline void bitset_init(Bitset* bs, u64* words, usize capacity) {
 ...
 * Performance: O(1)
 */
static inline void bitset_set(Bitset* bs, usize i) {
    require_msg(bs != NULL && i < bs->capacity, "bitset_set: index out of range");
    if (!bs || i >= bs->capacity) return;
 ...
 * Performance: O(1)
 */
static inline void bitset_clear(Bitset* bs, usize i) {
    require_msg(bs != NULL && i < bs->capacity, "bitset_clear: index out of range");
    if (!bs || i >= bs->capacity) return;
    bs->words[bitset_word_of(i)] &= ~bitset_mask_of(i);
}
 ...
 * Performance: O(1)
 */
static inline void bitset_toggle(Bitset* bs, usize i) {
    require_msg(bs != NULL && i < bs->capacity, "bitset_toggle: index out of range");
    if (!bs || i >= bs->capacity) return;
    bs->words[bitset_word_of(i)] ^= bitset_mask_of(i);
}
 ...
 * @pre i < bs->capacity
 * @return false if bs == NULL or i >= capacity
 * Performance: O(1)
 */
static inline bool bitset_test(const Bitset* bs, usize i) {
    if (!bs || i >= bs->capacity) return false;
 ...
 * Performance: O(1)
 */
static inline void bitset_assign(Bitset* bs, usize i, bool value) {
 ...
 * Performance: O(n/64)
 */
static inline void bitset_clear_all(Bitset* bs) {
 ...
 * Performance: O(n/64)
 */
static inline void bitset_set_all(Bitset* bs) {
    if (!bs || !bs->words) return;
 ...
 * Performance: O(n/64)
 */
static inline void bitset_not(Bitset* bs) {
    if (!bs || !bs->words) return;
 ...
 * Performance: O(n/64)
 */
static inline void bitset_and(Bitset* bs, const Bitset* other) {
    if (!bs || !other) return;
 ...
 * Performance: O(n/64)
 */
static inline void bitset_or(Bitset* bs, const Bitset* other) {
    if (!bs || !other) return;
    usize n = bs->word_count < other->word_count ? bs->word_count : other->word_count;
    for (usize w = 0; w < n; w++) {
        bs->words[w] |= other->words[w];
 ...
 * Performance: O(n/64)
 */
static inline void bitset_xor(Bitset* bs, const Bitset* other) {
    if (!bs || !other) return;
    usize n = bs->word_count < other->word_count ? bs->word_count : other->word_count;
    for (usize w = 0; w < n; w++) {
        bs->words[w] ^= other->words[w];
 ...
 *
 * Uses popcount64 from core/primitives/bits.h per word.
 * Performance: O(n/64)
 */
static inline usize bitset_count(const Bitset* bs) {
    if (!bs || !bs->words) return 0;
    usize total = 0;
    for (usize w = 0; w < bs->word_count; w++) {
        total += (usize)popcount64(bs->words[w]);
    }
    return total;
}
 ...
 * Performance: O(n/64)
 */
static inline bool bitset_is_empty(const Bitset* bs) {
 ...
 * Performance: O(n/64)
 */
static inline bool bitset_is_full(const Bitset* bs) {
 ...
 * Performance: O(n/64)
 */
static inline bool bitset_is_disjoint(const Bitset* bs, const Bitset* other) {
 ...
 * Performance: O(1)
 */
static inline usize bitset_capacity(const Bitset* bs) {
 ...
 *
 * Uses ctz64 (count trailing zeros) from core/primitives/bits.h.
 * Performance: O(n/64) worst case
 */
static inline usize bitset_find_first(const Bitset* bs) {
    if (!bs || !bs->words) return BITSET_NPOS;
    for (usize w = 0; w < bs->word_count; w++) {
        if (bs->words[w] != 0) {
            usize bit = w * BITSET_BITS_PER_WORD + (usize)ctz64(bs->words[w]);
 ...
 */
static inline usize bitset_find_next(const Bitset* bs, usize prev) {
    ...
    if (word != 0) {
        usize found = w * BITSET_BITS_PER_WORD + bit + (usize)ctz64(word);
        return found < bs->capacity ? found : BITSET_NPOS;
    }

    for (w++; w < bs->word_count; w++) {
        if (bs->words[w] != 0) {
            usize found = w * BITSET_BITS_PER_WORD + (usize)ctz64(bs->words[w]);
            return found < bs->capacity ? found : BITSET_NPOS;
        }
    }
    return BITSET_NPOS;
 ...
 *
 * Uses clz64 (count leading zeros) from core/primitives/bits.h.
 * Performance: O(n/64) worst case
 */
static inline usize bitset_find_last(const Bitset* bs) {
    if (!bs || !bs->words) return BITSET_NPOS;
    usize w = bs->word_count;
    while (w-- > 0) {
        if (bs->words[w] != 0) {
            usize bit = w * BITSET_BITS_PER_WORD +
                        (BITSET_BITS_PER_WORD - 1 - (usize)clz64(bs->words[w]));
            return bit < bs->capacity ? bit : BITSET_NPOS;
        }
    }
    return BITSET_NPOS;
 ...
 * Performance: O(1)
 */
static inline bytes_t bitset_as_bytes(const Bitset* bs) {
