...
/**
 * @brief Peeks at the next value as Option<ptrdiff_t>
 *
 * Safe alternative to range_peek() — returns None if empty or invalid.
 *
 * @param r Range to peek (NULL-safe)
 * @return Some(next) if available, None otherwise
 *
 * Performance: O(1)
 */
static inline option_ptrdiff_t range_peek_option(const range* r) {
    ptrdiff_t val;
    if (range_peek(r, &val)) {
        return option_ptrdiff_t_some(val);
    }
    return option_ptrdiff_t_none();
}

/**
 * @brief Gets the current value (without advancing) as Option<ptrdiff_t>
 *
 * Alias for range_peek_option — included for naming symmetry.
 */
#define range_current_option range_peek_option

/**
 * @brief Checks if the range is in a valid, non-exhausted state
 *
 * @param r Range to validate (NULL-safe)
 * @return true if range is non-NULL and has not been exhausted
 */
...
static inline ptrdiff_t range_next(range* r) {
    ...
    if (r->step > 0) {
        if (r->current > PTRDIFF_MAX - r->step) {
            r->current = r->end; // Exhaust on potential overflow
        } else {
            r->current += r->step;
            if (r->current >= r->end) {
                r->current = r->end;
        ...
        if (r->current < PTRDIFF_MIN - r->step) {
            r->current = r->end; // Exhaust on potential underflow
        } else {
            r->current += r->step;
            if (r->current <= r->end) {
                r->current = r->end;
...
/* Alias for common naming preference */
