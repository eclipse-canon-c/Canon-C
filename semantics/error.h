 ...
 *
 * Performance: Zero overhead - error codes are simple integers, messages are
 ...
 *
 * Example:
 *   int code = error_code(ERR_OUT_OF_MEMORY);
 ...
 *
 * @param T   Type name (e.g., int, float)
 * @param val Value to wrap in Ok
 *
 * Example:
 *   return RESULT_OK(int, 42);
 *   // Expands to: return result_int_Error_ok(42);
 */
 ...
 *
 * @param T        Type name (e.g., int, float)
 * @param err_code Error code (enum Error)
 *
 * Example:
 *   return RESULT_ERR(int, ERR_OUT_OF_MEMORY);
 *   // Expands to: return result_int_Error_err(ERR_OUT_OF_MEMORY);
 */
 ...
 *
 * Returns: Error message string if Err, NULL if Ok
 *
 * Example:
...
/* ────────────────────────────────────────────────────────────────────────────
   Custom error type pattern (for domain-specific needs)
   ────────────────────────────────────────────────────────────────────────────

    // Define your own error enum for specific domains
    typedef enum {
        HTTP_OK = 0,
    ...
    // Define Result type
    CANON_C_DEFINE_RESULT(Response, HttpError)
    
    // Create message function
    static inline const char* http_error_message(HttpError e) {
        switch (e) {
