...
## `bits.h` — Portable bit manipulation
...
## `checked.h` — Overflow-safe arithmetic
...
## `compare.h` — Comparator and predicate types
...
## `contract.h` — Contracts and assertions

Replaces `assert()` with semantic macros for preconditions, debug checks, and unreachable paths.
...
## `limits.h` — Numeric limits and constants

All compile-time, zero runtime cost.
...
## `ptr.h` — Named pointer arithmetic

All operations work on `void*` — no type punning needed. NULL-safe unless noted.
...
Foundation header. Include this everywhere.
