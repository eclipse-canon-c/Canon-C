...
**`vec_T smallvec_T_as_vec(smallvec_T* v)`**
Returns a zero-copy borrowed `vec_T` view over the current buffer. Does NOT transfer ownership — do not free or store beyond `v`'s lifetime. v must not be NULL — checked via `require_msg()`. Do not call any reallocating vec functions on the returned view.
```c
vec_int view = smallvec_int_as_vec(&v);
// pass to any API accepting vec_int
```
...
| Constructor | `_empty()` | `deque_##type` |
...
| What | Default name | Example for `int` |
|---|---|---|
| Type | `deque_T` | `deque_int` |
| Struct tag | `deque_T_s` | `deque_int_s` |
| `init` | `deque_T_init` | `deque_int_init` |
| `empty` | `deque_T_empty` | `deque_int_empty` |
| `len` | `deque_T_len` | `deque_int_len` |
| `capacity` | `deque_T_capacity` | `deque_int_capacity` |
| `remaining` | `deque_T_remaining` | `deque_int_remaining` |
| `is_empty` | `deque_T_is_empty` | `deque_int_is_empty` |
| `is_full` | `deque_T_is_full` | `deque_int_is_full` |
| `push_front` | `deque_T_push_front` | `deque_int_push_front` |
| `push_back` | `deque_T_push_back` | `deque_int_push_back` |
| `pop_front` | `deque_T_pop_front` | `deque_int_pop_front` |
| `pop_back` | `deque_T_pop_back` | `deque_int_pop_back` |
| `pop_front_option` | `deque_T_pop_front_option` | `deque_int_pop_front_option` |
| `pop_back_option` | `deque_T_pop_back_option` | `deque_int_pop_back_option` |
| `peek_front` | `deque_T_peek_front` | `deque_int_peek_front` |
| `peek_back` | `deque_T_peek_back` | `deque_int_peek_back` |
| `peek_front_option` | `deque_T_peek_front_option` | `deque_int_peek_front_option` |
| `peek_back_option` | `deque_T_peek_back_option` | `deque_int_peek_back_option` |
| `clear` | `deque_T_clear` | `deque_int_clear` |
| `swap` | `deque_T_swap` | `deque_int_swap` |
...
**`result_bool_Error hashmap_collect_keys(const hashmap* map, vec_K* out)`**
Iterates all occupied slots and appends each key into `out` using `vec_K_push()`. The vec is NOT cleared before insertion — keys are appended to whatever is already in it. Key ordering is unspecified.
...
**`result_bool_Error hashmap_collect_values(const hashmap* map, vec_V* out)`**
Iterates all occupied slots and appends each value into `out` using `vec_V_push()`. Value ordering corresponds to the order returned by `iter_next`. The vec is NOT cleared before insertion.
...
> - Push function names are derived directly from `HASHMAP_KEY_TYPE` and `HASHMAP_VAL_TYPE` via the `vec_##TYPE##_push` naming convention — types must be simple identifiers, not raw pointer types.
...
**`vec_T vec_T_init(T* buffer, usize capacity)`**
...
**`vec_T vec_T_arena_alloc(Arena* arena, usize capacity)`**
...
#### Queries
```c
vec_int_len(&v)        // usize — current element count
vec_int_capacity(&v)   // usize — maximum elements
vec_int_remaining(&v)  // usize — free slots (capacity - len)
vec_int_is_empty(&v)   // bool
vec_int_is_full(&v)    // bool
```
...
**`option_T vec_T_pop_option(vec_T* v)`**
Same but returns `Option<T>` — no out param. `None` if empty.

**`void vec_T_clear(vec_T* v)`**
Resets len to 0. Does NOT zero memory. NULL-safe. O(1).

**`result_bool_Error vec_T_insert(vec_T* v, usize i, T item)`**
Inserts at index `i`, shifts elements right. O(n).
- `Err(ERR_OUT_OF_RANGE)` — i > len
- `Err(ERR_CAPACITY_EXCEEDED)` — vec is full

**`result_bool_Error vec_T_remove(vec_T* v, usize i, T* out)`**
Removes at index `i`, shifts elements left. O(n).
- `Err(ERR_INVALID_STATE)` — vec is empty
- `Err(ERR_OUT_OF_RANGE)` — i >= len

**`option_T vec_T_remove_option(vec_T* v, usize i)`**
Same but returns `Option<T>`.

#### Bulk Operations

**`result_bool_Error vec_T_append_array(vec_T* v, const T* src, usize count)`**
...
slice_int sv = vec_int_as_slice(&v);       // typed view of [data, data+len)
slice_int sf = vec_int_as_slice_full(&v);  // typed view of [data, data+cap)
bytes_t   bv = vec_int_as_bytes(&v);       // raw byte view of used elements
```
...
typedef struct {
    vec_T* vec;  // pointer to parent vec
...
// sizeof(vec_T) = sizeof(T*) + 2*sizeof(usize)
...
Struct typedefs: `vec_T`, `vec_T_iter`, `vec_T_slice`
...
| Type | Description |
|---|---|
| `vec_T` | The vector struct (`items`, `len`, `capacity`) |
| `vec_T_iter` | Forward iterator (`vec*` + `index`) |
| `vec_T_slice` | Zero-copy sub-view (`items*` + `len`) |
...
// Forward iterator
typedef struct vec_T_iter_s {
    vec_T* vec;   // parent vec — invalidated by any modification
    usize        index; // current position (0 = start)
} vec_T_iter;

// Zero-copy sub-view
typedef struct vec_T_slice_s {
    T*    items; // pointer into parent vec's buffer (not owned)
...
|---|---|---|---|
| Types | Vector type | `vec_T` | `vec_int` |
| | Struct tag | `vec_T_s` | `vec_int_s` |
| | Iterator type | `vec_T_iter` | `vec_int_iter` |
| | Iterator tag | `vec_T_iter_s` | `vec_int_iter_s` |
| | Slice type | `vec_T_slice` | `vec_int_slice` |
| | Slice tag | `vec_T_slice_s` | `vec_int_slice_s` |
| Constructors | `init` | `vec_T_init` | `vec_int_init` |
| | `empty` | `vec_T_empty` | `vec_int_empty` |
| | `alloc` | `vec_T_alloc` | `vec_int_alloc` |
| | `arena_alloc` | `vec_T_arena_alloc` | `vec_int_arena_alloc` |
| | `free` | `vec_T_free` | `vec_int_free` |
| Queries | `len` | `vec_T_len` | `vec_int_len` |
| | `capacity` | `vec_T_capacity` | `vec_int_capacity` |
| | `remaining` | `vec_T_remaining` | `vec_int_remaining` |
| | `is_empty` | `vec_T_is_empty` | `vec_int_is_empty` |
| | `is_full` | `vec_T_is_full` | `vec_int_is_full` |
| Element access | `get` | `vec_T_get` | `vec_int_get` |
| | `get_option` | `vec_T_get_option` | `vec_int_get_option` |
| | `get_unchecked` | `vec_T_get_unchecked` | `vec_int_get_unchecked` |
| | `at` | `vec_T_at` | `vec_int_at` |
| | `set` | `vec_T_set` | `vec_int_set` |
| | `first` | `vec_T_first` | `vec_int_first` |
| | `last` | `vec_T_last` | `vec_int_last` |
| | `data` | `vec_T_data` | `vec_int_data` |
| Modification | `push` | `vec_T_push` | `vec_int_push` |
| | `try_push` | `vec_T_try_push` | `vec_int_try_push` |
| | `push_unchecked` | `vec_T_push_unchecked` | `vec_int_push_unchecked` |
| | `pop` | `vec_T_pop` | `vec_int_pop` |
| | `pop_option` | `vec_T_pop_option` | `vec_int_pop_option` |
| | `clear` | `vec_T_clear` | `vec_int_clear` |
| | `insert` | `vec_T_insert` | `vec_int_insert` |
| | `remove` | `vec_T_remove` | `vec_int_remove` |
| | `remove_option` | `vec_T_remove_option` | `vec_int_remove_option` |
| Bulk | `append_array` | `vec_T_append_array` | `vec_int_append_array` |
| | `extend` | `vec_T_extend` | `vec_int_extend` |
| | `fill` | `vec_T_fill` | `vec_int_fill` |
| | `swap` | `vec_T_swap` | `vec_int_swap` |
| Iterator | `iter_init` | `vec_T_iter_init` | `vec_int_iter_init` |
| | `iter_next` | `vec_T_iter_next` | `vec_int_iter_next` |
| Slice | `slice_init` | `vec_T_slice_init` | `vec_int_slice_init` |
| | `slice_get` | `vec_T_slice_get` | `vec_int_slice_get` |
...
#### `vec_T_to_stringbuf` — printf-style formatting

**`void vec_T_to_stringbuf(const vec_T* v, StringBuf* sb, const char* fmt)`**
...
#### `vec_T_to_stringbuf_cb` — callback-style formatting

**`void vec_T_to_stringbuf_cb(const vec_T* v, StringBuf* sb, void (*cb)(StringBuf*, T))`**
...
**`result_bool_Error vec_T_extend_from_range(vec_T* v, IntRange r)`**
...
vec_int_extend_from_range(&v, asc);
...
vec_int_extend_from_range(&key_vec, keys);
...
### `queue.h`
> Bounded FIFO queue — thin zero-overhead wrapper over `deque_##type`. enqueue = push_back, dequeue = pop_front, peek = peek_front. Fixed capacity, no automatic growth.
...
**`void queue_T_init(queue_T* q, type* buffer, usize capacity)`**
...
**`result_bool_Error queue_T_enqueue(queue_T* q, type item)`**
...
**`result_bool_Error queue_T_dequeue(queue_T* q, type* out)`**
...
**`option_T queue_T_dequeue_option(queue_T* q)`**
...
**`bool queue_T_peek(const queue_T* q, type* out)`**
Returns front item without removing it. Returns false if empty or invalid. Queue unchanged. O(1).

**`option_T queue_T_peek_option(const queue_T* q)`**
Returns front item as `Option<T>` without removing. Queue unchanged. O(1).

#### Queries
```c
queue_T_len(&q)        // usize — current element count
queue_T_capacity(&q)   // usize — fixed maximum
queue_T_remaining(&q)  // usize — free slots
queue_T_is_empty(&q)   // bool
queue_T_is_full(&q)    // bool
```
...
`queue_T` is a typedef alias for `deque_T` — no new struct. All `deque_T` functions remain usable. `DEFINE_QUEUE` only adds FIFO-named wrappers for clarity.
...
option_int front = queue_int_peek_option(&q);    // None — now empty
```
...
### `stack.h`
> Bounded LIFO stack — thin zero-overhead wrapper over `vec_##type`. push = vec_push, pop = vec_pop, peek = vec_last. Fixed capacity, no automatic growth.
...
**`void stack_T_init(stack_T* s, type* buffer, usize capacity)`**
...
**`result_bool_Error stack_T_push(stack_T* s, type item)`**
...
**`result_bool_Error stack_T_pop(stack_T* s, type* out)`**
...
**`option_T stack_T_pop_option(stack_T* s)`**
...
**`bool stack_T_peek(const stack_T* s, type* out)`**
Returns top item without removing it. Returns false if empty or invalid. Stack unchanged. O(1).

**`option_T stack_T_peek_option(const stack_T* s)`**
Returns top item as `Option<T>` without removing. Stack unchanged. O(1).

#### Queries
```c
stack_T_len(&s)        // usize — current element count
stack_T_capacity(&s)   // usize — fixed maximum
stack_T_remaining(&s)  // usize — free slots
stack_T_is_empty(&s)   // bool
stack_T_is_full(&s)    // bool
```
All NULL-safe — return 0 or true if `s == NULL`.

#### Misc

**`void stack_T_clear(stack_T* s)`**
Resets stack to empty. O(1). Does NOT zero buffer contents. NULL-safe.

#### Type Layout

`stack_T` is a typedef alias for `vec_T` — no new struct. All `vec_T` functions remain usable. `DEFINE_STACK` only adds LIFO-named wrappers for clarity.
...
option_int peek = stack_int_peek_option(&s); // None — now empty
```
...
### `search.h`
> Binary search utilities for sorted sequences. O(log n) for all operations. Requires input to be sorted with the same comparator used for searching. All functions panic on NULL pointers and `elem_size == 0` — `len == 0` is always valid.
...
// range[0] = 1, range[1] = 4 — all 2's at indices [1, 4)

int missing = 3;
algo_equal_range(arr, 7, sizeof(int), &missing, algo_cmp_int, NULL, range);
// range[0] = 4, range[1] = 4 — empty range, key not present

algo_equal_range(arr, 0, sizeof(int), &key, algo_cmp_int, NULL, range);
// range[0] = 0, range[1] = 0 — empty array
```

---

**`bool algo_binary_search(const void* array, usize len, usize elem_size, const void* key, algo_cmp_fn cmp, void* ctx)`**
Boolean existence check — returns `true` if at least one element compares equal to key. Returns `false` for empty arrays.
```c
int numbers[] = {1, 3, 5, 7, 9, 11};
int key = 7;
bool found  = algo_binary_search(numbers, 6, sizeof(int), &key, algo_cmp_int, NULL);
// found = true

int missing = 6;
bool found2 = algo_binary_search(numbers, 6, sizeof(int), &missing, algo_cmp_int, NULL);
// found2 = false
```
